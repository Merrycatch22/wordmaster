...
hintlines=open("hints").read().splitlines()# only one extra param now
hintlist=[]
...
for word in wlist:
    ...
    for hint in hintlist:
        ...
        if plus1+2*plus2!=int(hint[1]): 
            #modified for evil, the plusses added for generalization of hints
            #need to double plus2 cuz worth more
            flag=False
            break
    if flag:
        listok.append(word)
